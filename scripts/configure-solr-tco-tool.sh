...
SOLR_DOWNLOAD="http://ftp.drupal.org/files/projects/search_api_solr-7.x-1.12.tar.gz"
SOLR_DOWNLOAD_DIR="/tmp"
SOLR_MODULE_NAME="search_api_solr"
SOLR_VERSION="5.x"
SOLR_CORE_PATH="/var/solr/data/k00295sr01"

# Check to see if we've already performed this setup.
if [ ! -e "$SOLR_SETUP_COMPLETE_FILE" ]; then
  # Create the required folder structure.
  sudo mkdir $SOLR_CORE_PATH
  sudo mkdir $SOLR_CORE_PATH/conf
  sudo mkdir $SOLR_CORE_PATH/data
  sudo chown solr:solr $SOLR_CORE_PATH/data
  sudo chown solr:solr $SOLR_CORE_PATH/conf
